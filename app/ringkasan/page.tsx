...
export default function RingkasanPage() {
  return (
    <section className="space-y-10">
      <header>
        <p className="text-sm text-slate-500">Insight untuk seluruh perjalanan</p>
        <h1 className="text-3xl font-bold text-slate-900">Ringkasan komunitas</h1>
      </header>
      ...
      <div className="rounded-2xl border bg-white/90 p-6 shadow-sm">
        ...
        <p className="text-sm text-slate-500">Cukup selesaikan transfer sesuai daftar ini.</p>
        <ul className="mt-4 space-y-3">
          {ringkasan.rekomendasi.map((item) => (
            <li key={`${item.dari}-${item.ke}`} className="flex items-center justify-between rounded-xl border px-4 py-3">
              <p>
                <span className="font-semibold">{item.dari}</span> → <span className="font-semibold">{item.ke}</span>
              </p>
              <p className="font-semibold text-brand-blue">{formatRupiah(item.amount)}</p>
            </li>
          ))}
        </ul>
      </div>
