...
export function HostControls({ tripId, participants, expenses, adjustments }: HostControlsProps) {
  ...
  const handleAdjustmentAction = (adjustmentId: string, action: "apply" | "void") => {
    startTransition(async () => {
                ...
                adjustments.map((adjustment) => (
                  <li key={adjustment.id} className="rounded-xl border px-3 py-2">
                    ...
                    <p className="text-xs text-slate-400">
                      ...
                      {adjustment.appliedAt && ` · Dilunas ${format(new Date(adjustment.appliedAt), "d MMM HH:mm", { locale: localeId })}`}
                    </p>
