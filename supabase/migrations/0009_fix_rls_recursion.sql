...
-- The issue: trips policy checks participants, participants policy checks trips -> infinite loop
-- Solution: Only allow trip owners to see everything, disable RLS temporarily for simple auth
...
-- Trips: Only owner can see and manage (no participant check to avoid recursion)
...
-- Expenses: Only owners can access
CREATE POLICY "owners access expenses" ON public.expenses
  FOR ALL USING (
    EXISTS (SELECT 1 FROM public.trips t WHERE t.id = trip_id AND t.owner_id = auth.uid())
  );

-- Expense splits: Only owners can access
...
-- Settlements: Only owners can access
CREATE POLICY "owners access settlements" ON public.settlements
  FOR ALL USING (
    EXISTS (SELECT 1 FROM public.trips t WHERE t.id = trip_id AND t.owner_id = auth.uid())
  );

-- Trip invites: Only owners can manage
...
-- Host payment accounts: Only owners can manage
